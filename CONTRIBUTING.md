...
4. If you add or edit a vignette that contains mathematics, keep it body-sized
   on both render surfaces (the standalone `html_vignette` MathJax and the
   pkgdown-site KaTeX). Mirror the YAML of `vignettes/janusplot.Rmd`
   (`mathjax:` + `--include-in-header=mathjax-config.html`) and verify with:

   ```sh
   Rscript tools/check_vignette_math.R
   ```

5. Update `NEWS.md` under `# janusplot (development version)` with a bullet
